...
Portanto a Tabela 2 \@ref(tab:tab1) informa que depois do banco ser ajustado, o anime
...
Todo ano existem animes que se destacam, sejam lançamentos ou sejam temporadas novas, dessa maneira é interessante verificar influência da popularidade no *score*, o que podemos ver pela Figura \@ref(fig:fig-top)
...
```
Podemos notar então que aqueles que se destacam para a comunidade em termos gerais possuem *score* maior, um fato importante para a modelagem do score em seções seguintes.
...
```{r}
fit = betareg(score ~ ., 
        data = df |> select(-title, -ranked, -popularity), x=TRUE)
```
...
A^[Cuidado se você consegue identificar algum padrão nesse gráfico]
```{r}
...
\section{\centering No final, é sempre sobre amizade}
